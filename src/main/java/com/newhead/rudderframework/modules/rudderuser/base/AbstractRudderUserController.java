...
public abstract class AbstractRudderUserController extends WebController  {
     ...
     * @param id
     */
    @ApiOperation(value = "删 除", httpMethod = "POST", response = String.class)
    ...
    public ApiEntity getPage(@RequestParam(required = false) String rudderuserName,@RequestParam(required = false) String status,@RequestParam(required = false) Integer page,@RequestParam(required = false) Integer size) {
        ...
        Page<List<SimpleRudderUserQueryResponse>> sources = getService().queryPage(request);
