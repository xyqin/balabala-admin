...
public abstract class AbstractRudderPermissionController extends WebController  {
    ...
    public ApiEntity getPage(@RequestParam(required = false) String rudderpermissionName,@RequestParam(required = false) Integer page,@RequestParam(required = false) Integer size) {
        ...
        Page<List<SimpleRudderPermissionQueryResponse>> sources = getService().queryPage(request);
