...
public abstract class IntrospectedTable {
    ...
    protected enum InternalAttribute {
        ...
        /** The attr api implementation type. */
        ATTR_DAO_IMPLEMENTATION_TYPE,
        
        /** The attr api interface type. */
    ...
    protected String calculateJavaClientImplementationPackage() {
        ...
        if (stringHasValue(config.getImplementationPackage())) {
            ...
            String s = StringUtils.replace(config.getTargetPackage(),"$moduleName",fullyQualifiedTable.getDomainObjectName().toLowerCase());
            if (this.getFullyQualifiedTable().getDomainObjectName().toLowerCase().contains("rudder")) {
                s = StringUtils.replace(s,"$projectName","rudderframework");
            } else {
                s = StringUtils.replace(s,"$projectName",config.getProjectName());
            }
            sb.append(s);
        }
    ...
    protected String calculateJavaClientInterfacePackage() {
        ...
        StringBuilder sb = new StringBuilder();
        String s = StringUtils.replaceOnce(config.getTargetPackage(),"$moduleName",fullyQualifiedTable.getDomainObjectName().toLowerCase());
        if (this.getFullyQualifiedTable().getDomainObjectName().toLowerCase().contains("rudder")) {
            s = StringUtils.replace(s,"$projectName","rudderframework");
        } else {
            s = StringUtils.replace(s,"$projectName",config.getProjectName());
